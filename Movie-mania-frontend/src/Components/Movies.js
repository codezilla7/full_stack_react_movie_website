...
export default function Movies() {
    ...
    return (
        <div className='movies-mother-div'>
            <h1 className='nowstreaming'>Now Streaming</h1>
            <div className='movies-main-div'>
                {movies.length === 0 ? <p className='NMF' style={{ height: "60vh", textAlign: "center" }}><h1 style={{ display: 'inline', margin: "10px" }}><i class="bi bi-exclamation-circle-fill"></i></h1>{`Sorry, we couldn't find the matching movie.`}</p> : movies.map(film => {
                    return (
                        <Link to={`/moviedetails/${film._id}`} className='movies-card'>
                            <img alt={`couldn't load`} src={film.Poster} className='movies-poster'></img>
                            <div className='movies-detail'>
                                <p className='movies-title'>{film.Title}</p>
                                <p className='movies-runtime'>Runtime: {film.Runtime}</p>
                                <p className='movies-year'>Released: {film.Year}</p>
            ...
            {movies.length === 0 ? <Pagination style={{ display: "none" }} /> : <Pagination current={current} onChange={onChange} total={movies.length} />}
        </div>
    )
}